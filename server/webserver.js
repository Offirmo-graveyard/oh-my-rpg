...
app.set('views', path.join(__dirname, '/views')); // views directory : default to /views
...
if(env === 'development') {
	// WARNING
	// It has been found to slow down extremely the app at startup. (is it my computer ?)
	// Can even cause browser timeouts on first load.
	// Just refresh the page a second time and it should work.
...
app.use(favicon_server(path.join(__dirname, '../client/default/public/favicon.ico')));
...
// extend first, then default
app.use(express.static(path.join(__dirname, '../client/extend/public')));
app.use(express.static(path.join(__dirname, '../client/default/public')));
app.use('/common', express.static(path.join(__dirname, '../common/extend')));
app.use('/common', express.static(path.join(__dirname, '../common/default')));
